...
class InstagramBlockBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if (empty($this->configuration['access_token'])) {
      $this->loggerFactory->get('instagram_block')->error($this->t('No access_token set'));
      return $build;
    }
  ...
  protected function fetchData($url) {
    try {
      ...
      if (empty($data)) {
        $this->loggerFactory->get('instagram_block')->error($this->t('Invalid JSON or empty body returned by Instagram'));
        return FALSE;
      }
      ...
      $this->loggerFactory->get('instagram_block')->error($e->getMessage());
      return FALSE;
    }
  }
